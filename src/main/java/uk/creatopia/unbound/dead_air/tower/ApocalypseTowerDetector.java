...
public class ApocalypseTowerDetector {
    ...
    public static void initialize() {
        ...
        for (String id : radioPanelIds) {
            ...
            if (block != null) {
                ...
                Dead_air.LOGGER.info("Registered RadioTowers radio panel block: {}", id);
            }
        }
        ...
        for (Map.Entry<net.minecraft.resources.ResourceKey<Block>, Block> entry : ForgeRegistries.BLOCKS.getEntries()) {
            ...
            if (RADIOTOWERS_MODID.equals(blockId.getNamespace())) {
                ...
                if (path.contains("radio_panel") || path.contains("panel")) {
                    ...
                    Dead_air.LOGGER.info("Registered RadioTowers radio panel block: {}", blockId);
                }
            }
        }
        ...
        if (RADIO_PANEL_BLOCKS.isEmpty()) {
            ...
            Dead_air.LOGGER.info("Detected {} RadioTowers radio panel block(s)", RADIO_PANEL_BLOCKS.size());
        }
        
        Dead_air.LOGGER.info("RadioTowers mod integration initialized. Will detect towers by structure scanning and Radio Panel detection.");
    }
    ...
    public static ApocalypseTowerType getTowerType(ServerLevel level, BlockPos pos) {
        ...
        // Check for overrun tower characteristics
        // Overrun towers might have different block patterns or be in different biomes
        // For now, we'll use a simple heuristic: if there are many radiotowers blocks
        // but no metal_bars, and the structure seems larger, it might be overrun
        // But since we can't easily distinguish, we'll default to standard
    
        
        // Default: if we found a Radio Panel but no metal_bars, assume standard tower
        // (Standard towers are the most common)
        // Note: Overrun towers will need better detection - for now they'll be detected as STANDARD
        // and can be manually set to OVERRUN if needed, or we can improve detection later
