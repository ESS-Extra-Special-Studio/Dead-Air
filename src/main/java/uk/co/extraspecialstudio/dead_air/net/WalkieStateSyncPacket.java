...
/**
 * Client → Server: apply the active walkie's tune/power/link NBT and untune every other walkie.
 * Required so Static Note drops (server-side) see the same active radio as client audio.
 */
public class WalkieStateSyncPacket {
    /** Inventory slot in {@code player.getInventory().items}, or -1 for offhand. */
    ...
    public static void encode(WalkieStateSyncPacket msg, FriendlyByteBuf buf) {
    ...
    public static WalkieStateSyncPacket decode(FriendlyByteBuf buf) {
    ...
    public static void handle(WalkieStateSyncPacket msg, Supplier<NetworkEvent.Context> ctxSupplier) {
